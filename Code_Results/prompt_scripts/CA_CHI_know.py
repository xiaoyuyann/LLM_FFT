...
Here are the statistical facts from CMAP:
{community_info}
...
def load_community_info(community: str) -> str:
    """
    Loads the extra community info from the CA_info folder for the given community.
    Returns a pretty-printed JSON string, or a message if not found.
    """
    safe_name = community.replace("'", "").replace("’", "").replace("‘", "").replace(" ", "_")
    info_path = Path("CA_info") / f"{safe_name}.json"
    if info_path.exists():
        try:
            ...
            return f"(Error loading community info: {e})"
    else:
        return "(No extra community info found for this area.)"

def get_agent_response(community: str) -> dict | None:
    try:
        community_info = load_community_info(community)
        user_prompt = USER_PROMPT_TEMPLATE.format(community=community, community_info=community_info)
...
if __name__ == "__main__":
    batch_main(batch_version="CA_CHI_4o_info", runs=10) 